    ...
    def _compute_name_by_sequence(self):
        for move in self:
            ...
            if (
                ...
                # => we use with_context(ir_sequence_date=date, model_name='account.move').next_by_id()
                # which applies only for account.move on ir.sequence.date_range selection AND prefix
                name = seq.with_context(ir_sequence_date=move.date, model_name='account.move').next_by_id()
            move.name = name
